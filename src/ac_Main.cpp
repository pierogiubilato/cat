...
// Application Global (ag) settings.
Uint32 cat::ag::_Verbose = PEAR_VERB_WARN;		// No verbosity by default.
...
std::string cat::ag::_PathMain = "";			// No deafult path.	
std::string cat::ag::_PathFonts = "";			// No deafult font path.	
...
cat::ac::server* pear::ac::_server = 0;			// No inst. Server.
cat::ac::loop* pear::ac::_loop = 0;				// No inst. Loop.

// Global application facility services.
cat::af::file* pear::af::_file = 0;				// No inst. File Facility.
cat::af::font* pear::af::_font = 0;				// No inst. Font Facility.
...
int main_ServStart() 
{
	...
	// --------------------------
	try {
		pear::af::_font = new pear::af::font(cat::ag::_PathFonts);
	}
	catch (const std::exception& ex) {
		std::cout << "cat: " << ex.what() << "\n";
		return 1;
	...
	// -----------------------
	try {
		cat::ac::_Server = new pear::ac::server(2345);
	}
	catch (const std::exception& ex) {
		std::cout << "cat: " << ex.what() << "\n";
		return 1;
